...
def apply_associative_law(expression):
    ...
    for transform in transformations:
        print(transform)
        print_expression_table(transform)
            ...
            while token_i < len(tokens_for_tree):
                if token_i < 2:
                    if tokens_for_tree[token_i] == "/" and tokens_for_tree[token_i + 1] != "(":
                        tokens_for_tree = [[tokens_for_tree[token_i - 1], "/", tokens_for_tree[token_i + 1]]] + tokens_for_tree[token_i + 2:]
                        token_i += 1
                token_i += 1
...
if __name__ == "__main__":
    expression = "a*b+a*c+d*(e-f)+g*(f-e)+(x-y)*(v+k+4)+(l+r)/(m+n-p)+n+b*k+b*t"
    print(f"Expression:\n{expression}")
